...
const Step3Report = ({report}) => {
  ...
  const downloadPDF = () => {
    ...
    doc.line(margin,currentY+2,pageWidth-margin,currentY + 1)
    ...
    <div className='min-h-screen bg-linear-to-br from-gray-50 to-green-50 px-4 sm:px-6 lg:px-10 py-8'>
      <div className='mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
        <div className='md:mb-10 w-full flex items-start gap-4'>
          ...
          ><FaArrowLeft className='text-gray-600'/></button>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 '>
        <div className='space-y-6'>
          ...
          className='bg-white rounded-2xl sm:rounded-3xl shadow-lg p-6 sm:p-8 text-center'>
            ...
            <div className='relative w-36 h-36 sm:w-25 sm:h-25 mx-auto'>
              <CircularProgressbar
              value={percentage} 
              text={`${score}/10`}
              styles={buildStyles({
              ...
              {questionWiseScore.map((q,i)=>(
                <div key={i} className='bg-gray-50 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200'>
                  ...
                  <div className='bg-green-50 border border-green-200 p-4 rounded-lg'>
                    ...
                    <p className='text-sm text-gray-700 leading-relaxed'>{q.feedback && q.feedback.trim() != "" ? q.feedback:"No feedback available for this question."}</p>
                  </div>
                </div>
              ))}
            </div>
